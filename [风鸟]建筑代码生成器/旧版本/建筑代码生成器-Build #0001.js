...
function getCode(jx,jy,jz,filterss)
{
	...
	var code=new Array(),endcode="",lastblocks=new Array();
...
function switchData(data)
{
	...
	case"getcode":
		...
		var code=getCode(x3,y3,z3,ftt);
		var nowcode="function fixCode(jx,jy,jz){var code=["+code+"],a=0,i=0;for(i=0;i<code.length;i=i+5){Level.setTile(parseInt(code[i+0])+jx,parseInt(code[i+1])+jy,parseInt(code[i+2])+jz,parseInt(code[i+3]),parseInt(code[i+4]));a++;}return a;}";
		selectDialog("建筑代码生成器 - 生成完毕 - 共"+blocks+"个方块","请选择需要进行的操作",android.R.drawable.stat_notify_chat,"复制为纯建筑代码","复制为可调用代码","clip.setText(\"["+code+"]\");print(\"已复制到剪贴板\");","clip.setText(\""+nowcode+"\");print(\"已复制到剪贴板\");");
		break;
	case"fix":
		if(x3==undefined)
		{
