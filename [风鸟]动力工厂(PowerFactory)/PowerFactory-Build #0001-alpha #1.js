...
//ModPE.overrideTexture("images/terrain-atlas.tga","file:///sdcard/a/t.png");
//ModPE.overrideTexture("images/terrain.meta","file:///sdcard/a/terrain.meta");

//ID表：太阳能发电机，电池，挖矿机，粉碎机
var bids=[180,181,182,183];
var miniList=[[1,4],[2,3],[3],[4],[12],[13],[14],[15],[16,263],[21,351,4,6],[48],[49],[56,264],[73,331,0,6],[74,331,0,6],[82,337,0,4],[24],[121],[110,3],[129,388],[243,3]];//矿物定义表
var smashList=[[1,57,0,2]];//粉碎机物品表
...
function useItem(x,y,z,itemId,blockId,side,itemData,blockData)
{
	...
	if(itemId==280)
	{
		...
		case bids[2]:
			...
			break;
		case bids[3]:
			for(var i=0;i<pulverizers.length;i++)
			{
				if(pulverizers[i].x==x && pulverizers[i].y==y && pulverizers[i].z==z)
				{
					pulverizers[i].leftTime=99999;
					pulverizers[i].outputItem=findSmashItem(itemId,itemData);
					pulverizers[i].smashing[0]=itemId;
					pulverizers[i].smashing[1]=itemData;
					pulverizers[i].smashing[2]=Player.getCarriedItemCount()-1;
					pulverizers[i].leftTime=pulverizers[i].outputItem[3];
					pulverizers[i].outputItem.pop();
					clientMessage(pulverizers[i].outputItem);
					clientMessage(pulverizers[i].smashing);
					clientMessage(pulverizers[i].leftTime);
				}
			}
			break;
		}
...
function modTick()
{
	for(var i=0;i<solar_generator.length;i++)
	{
		...
		else if(Math.sin(Level.getTime()/12000*Math.PI)<=0)
		{
			break;
		}
		else
						...
						for(var z=mining_machines[i].z-5;z<mining_machines[i].z+5;z++)
						{
							...
							for(var w=0;w<miniList.length;w++)
							{
								if(blockId==miniList[w][0])
								{
									...
									if(dropId!=0)
									{
										Level.dropItem(x+0.5,mining_machines[i].dy,z+0.5,0.1,dropId,num,data);
									}
	...
	for(var i=0;i<pulverizers.length;i++)
	{
		if(Level.getTile(pulverizers[i].x,pulverizers[i].y,pulverizers[i].z)!=bids[3])
		{
			pulverizers.splice(i,1);
		}
		else if(pulverizers[i].outputItem[0]!=-1)
		{
			var powerId=getPower(pulverizers[i].x,pulverizers[i].y,pulverizers[i].z);
			for(var ii=0;ii<powerId.length;ii=ii+2)
			{
				if(powerId[ii]!=-1 && powerId[ii+1]=="solar_generator")
				{
					if(solar_generator[powerId[ii]].power>=0.1)
					{
						solar_generator[powerId[ii]].power=solar_generator[powerId[ii]].power-0.1;
						pulverizers[i].leftTime--;
						if(pulverizers[i].leftTime<=0)
						{
							var num=addItemChest(pulverizers[i].x,pulverizers[i].y+1,pulverizers[i].z,pulverizers[i].outputItem[0],pulverizers[i].outputItem[1],pulverizers[i].outputItem[2])
							if(num>0)
							{
								Level.dropItem(pulverizers[i].x+0.5,pulverizers[i].y,pulverizers[i].z+0.5,0.1,pulverizers[i].outputItem[0],num,pulverizers[i].outputItem[1]);
							}
							if(pulverizers[i].smashing[2]>0)
							{
								pulverizers[i].outputItem=findSmashItem(pulverizers[i].smashing[0],pulverizers[i].smashing[1]);
								pulverizers[i].smashing[2]--;
								pulverizers[i].leftTime=pulverizers[i].outputItem[3];
								pulverizers[i].outputItem.pop();
							}
							else
							{
								pulverizers[i].outputItem=[-1,0,0];
							}
						}
...
function findSmashItem(id,data)
{
	return [57,0,2,200];
}
function addItemChest(x,y,z,id,data,count)
{
	...
	for(var i=0;i<27;i++)
	{
		...
		if((Level.getChestSlot(x,y,z,i)==id && Level.getChestSlotData(x,y,z,i)==data) || Level.getChestSlot(x,y,z,i)==0)
		{
			add=64-Level.getChestSlotCount(x,y,z,i);
			count=count-add;
			Level.setChestSlot(x,y,z,i,id,data,Level.getChestSlotCount(x,y,z,i)+add);
		}
		if(count<=0)
		{
			return 0;
...
function saveMachines()
{
	...
	{
		tmpString=tmpString+"{x:"+mining_machines[i].x+",y:"+mining_machines[i].y+",z:"+mining_machines[i].z+",on:"+mining_machines[i].on+",leftTime:"+mining_machines[i].leftTime+",dy:"+mining_machines[i].y-1+"}";
...
function catchmsg(id,msg)//报错
{
	function cth(msge)
	{
		selectDialog("动力工厂 - "+build+" - 出现错误",msge,android.R.drawable.stat_notify_error,"确定","退出存档","","ModPE.leaveGame()");
		clientMessage("动力工厂 - "+build+" - 出现错误\n"+msge);
	}
	switch(id)
	{
	case 10:
...
//Block.newBlock(bids[3],"粉碎机",[["piston_top_normal",0],["piston_bottom",0],["piston_top_sticky",0],["piston_top_sticky",0],["piston_top_sticky",0],["piston_top_sticky",0]],20,0.8,5,1,-1,true);
