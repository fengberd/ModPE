	...
	for(var i=0;i<pulverizers.length;i++)
	{
		...
		else if(pulverizers[i].outputItem[0]!=-1)
		{
			...
			for(var ii=0;ii<powerId.length;ii=ii+2)
			{
				if(powerId[ii]!=-1 && powerId[ii+1]=="solar_generator")
				{
					if(solar_generator[powerId[ii]].power>=0.1)
					{
						...
						if(pulverizers[i].leftTime<=0)
						{
							...
							if(num>0)
							{
								Level.dropItem(pulverizers[i].x+0.5,pulverizers[i].y+0.5,pulverizers[i].z+0.5,0.3,pulverizers[i].outputItem[0],num,pulverizers[i].outputItem[1]);
							}
				...
				if(powerId[ii]!=-1 && powerId[ii+1]=="batbox")
				{
					if(batBoxs[powerId[ii]].power>=0.1)
					{
						batBoxs[powerId[ii]].power=batBoxs[powerId[ii]].power-0.1;
						pulverizers[i].leftTime--;
						if(pulverizers[i].leftTime<=0)
						{
							var num=addItemChest(pulverizers[i].x,pulverizers[i].y+1,pulverizers[i].z,pulverizers[i].outputItem[0],pulverizers[i].outputItem[1],pulverizers[i].outputItem[2])
...
function addItemChest(x,y,z,id,data,count)
{
	...
	for(var i=0;i<27;i++)
	{
		...
		if((Level.getChestSlot(x,y,z,i)==id && Level.getChestSlotData(x,y,z,i)==data) || Level.getChestSlot(x,y,z,i)==0)
		{
			...
			if(add>=count)
			{
				Level.setChestSlot(x,y,z,i,id,data,Level.getChestSlotCount(x,y,z,i)+count);
				count=0;
			}
			else
			{
