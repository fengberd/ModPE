/*本js由风鸟制作！严禁盗版或修改版权！！
BUG反馈及功能建议搜索群“风鸟神教”加入提出

备忘：
		为毛entityremovehook不能检测到玩家死亡？！！
		发布前去除被屏蔽的代码
*/
//********************************变量声明区开始********************************
var ctx = com.mojang.minecraftpe.MainActivity.currentMainActivity.get();//公用ctx
var build="Build #2073";
var first=false;//是否第一次运行
var god=false//无敌
var skill=false//秒杀
var debug=false,debugWindow=null,dtext=null;//调试模式
...
var saveworld=false,savemob=false;//保护模式
var PlayerEnt;//玩家实体ID
var sneaking=false;//潜行模式
var magnet=0,list;//磁铁模式
var username="";//用户名
var sandnodown=false;//沙子/砂砾不掉落
//GUI区
...
function attackHook(a,e)//生物被攻击
{
	if(!savemob)
	{
		if(skill==true && Entity.getHealth(e) >= 2 && e != PlayerEntity )
		{
...
function modTick()//0.05秒执行一次
{
	...
	if(magnet!=0)
	{
		...
		for(i=0;i<list.length;i++)
		{
			if(Entity.getEntityTypeId(list[i])==64 || Entity.getEntityTypeId(list[i])==66)
			{
				if(magnet==-1)
				{
	...
	{
		var tmpx,tmpy,tmpz;
		tmpx=Entity.getX(PlayerEntity)+".";
		tmpx=tmpx.split(".");
		tmpy=Entity.getY(PlayerEntity)+".";
		tmpy=tmpy.split(".");
		tmpz=Entity.getZ(PlayerEntity)+".";
		tmpz=tmpz.split(".");
		dtext.setText("\nX:"+tmpx[0]+"."+tmpx[1][0]+tmpx[1][1]+tmpx[1][2]+"\nY:"+tmpy[0]+"."+tmpy[1][0]+tmpy[1][1]+tmpy[1][2]+"\nZ:"+tmpz[0]+"."+tmpz[1][0]+tmpz[1][1]+tmpz[1][2]+ "\nEntity:"+ PlayerEntity +"\nID:"+Player.getCarriedItem()+":"+Player.getCarriedItemData());
	}catch(err){
			catchmsg(12,err);
		}}}))};
}
function leaveGame()//退出存档
{
	...
	saveworld=false,savemob=false;//保护模式
	PlayerEnt;//玩家实体ID
	sneaking=false;//潜行模式
	magnet=0;//磁铁模式
	sandnodown=false;//沙子/砂砾不掉落
	//username="";//玩家识别系统
	ctx.runOnUiThread(new java.lang.Runnable({ run: function() {
...
function usecmd(cmd)//模拟调用指令
{
	...
	/////////////////////////////////////////////////////////////////爆炸
	case"explode":
		var expx = Entity.getX(PlayerEntity);
		var expy = Entity.getY(PlayerEntity);
		var expz = Entity.getZ(PlayerEntity);
		var nowHP = Entity.getHealth( PlayerEntity );
		Entity.setHealth(PlayerEntity,32767);
		if(Data.length==1)
		{
			explode(expx, expy, expz, 4.0);
			usecmd("explode 4");
		}
		else
		{
			explode(expx,expy,expz,Data[1]);
			...
			ctx.runOnUiThread(new java.lang.Runnable(
			{
				run:function()
				{
					try
					{
						...
						debugWindow.setWidth(dip2px(ctx, 70));
			...
			button.setOnClickListener(new android.view.View.OnClickListener({
				onClick: function(viewarg)
				{
					var Menus_MainMenu=new Array(
						...
						{kind:"button",text:"打开物品生成菜单",data:"ITEMMENU"},
						{kind:"button",text:"打开玩家操作菜单",data:"PLAYERMENU"},
						{kind:"button",text:"打开实体操作菜单",data:"MOBMENU"},
						{kind:"button",text:"打开时间操作菜单",data:"TIMEMENU"},
						{kind:"button",text:"打开定点传送菜单",data:"TPMENU"},
						{kind:"text",text:"辅助模式：",color:"",size:20},
...
function switchData(data)//GUI判断指令
{
	...
	case"TIMEMENU":
		var Menus_TimeMenu=new Array(
			...
			{kind:"button",text:"变成白天",data:"morning"},
			{kind:"button",text:"变成夜晚",data:"night"},
			{kind:"button",text:"设置时间_Dialog",data:""},
	...
	case"MOBMENU":
		var Menus_MobMenu=new Array(
			...
			{kind:"check",text:"防止沙/砂砾掉落",data:"sandnodown",torf:sandnodown},
			{kind:"button",text:"磁铁模式",data:"MAGNET"},
			{kind:"button",text:"世界级磁铁模式",data:"MAGNET-1"});
	...
	case"MAGNET-1":
		...
		break;
	case"SPEED":
		inputDialog(true,false,"","请输入速度","","","变速","确定","取消","speed ");
		break;
	case"PLAYERSET":
...
function inputDialog(ttext1,ttext2,text1,htext1,text2,htext2,title,btn1text,btn2text,data)//输入型Dialog
{
	try{
		...
		layout.addView(btn1);
		layout.addView(btn2);
		var Dialogw = new android.app.Dialogw(ctx);
...
function Menu_open(MenuData)//主函数
{
	try{
		...
		scrollview.setLayoutParams(new android.widget.RelativeLayout.LayoutParams(android.widget.RelativeLayout.LayoutParams.FILL_PARENT, android.widget.RelativeLayout.LayoutParams.FILL_PARENT));//变量
