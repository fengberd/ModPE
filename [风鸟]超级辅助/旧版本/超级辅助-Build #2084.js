...
function chatHook(c)
{
	if(c=="233")
	{
		CM("<Herobine>"+getColor("yellow")+"2333333333","white");
	}
}
...
function newLevel()//进入世界
{
	...
	if(load("printmode")=="true")
	{
		printmode=true;
	}
	if(load("no_colormode")=="true")
	{
		no_colormode=true;
	}
	//载入按钮
...
function catchmsg(id,msg)//GUI报错
{
	function cth(msge)
	{
		selectDialog("超级辅助 - "+build+" - 出现错误","错误信息:\n"+msge,android.R.drawable.stat_sys_warning,"确定","退出存档","","ModPE.leaveGame()");
...
function usecmd(cmd)//核心函数
{
	...
	case"printmode":
		printmode=!printmode;
		save("printmode",printmode);
		break;
	/////////////////////////////////////////////////////////////////文字无色模式
	case"no_colormode":
		no_colormode=!no_colormode;
		save("no_colormode",no_colormode);
		break;
	/////////////////////////////////////////////////////////////////没找到
	default:
...
function switchData(data)//GUI判断指令
{
	...
	case"MOREMENU":
		...
		break;
	case"DEBUGNOW":
		inputDialog(true,false,"","输入欲执行的指令","","","Eval动态代码执行系统","确定","取消","eval ");
		break;
	case"TPP":
